         ...
         //int argc, char const *argv[])
{
	...
	while(!feof(archivoDeEntrada)){   // Mientras no termine el archivo de entrada
		...
		fread(&nextChar, 2, 1, archivoDeEntrada);
		while (nextChar != ' '){
            		currentStatus = productions [currentStatus][clasificarChar(nextChar)];
            		fwrite(&nextChar,2, 1, archivoDeSalida);
			fread(&nextChar,2, 1, archivoDeEntrada);
		};
        	printf ("Read/write");
		...
		if (currentStatus != 0){
				...
				case 4:
				case 6:
					currentWord = "    No es una consante entera";
				break;
			};

		       	fwrite(currentWord ,2, strlen(currentWord), archivoDeSalida);
		};
